...
export const vocationalApi = {
  ...
  viewSilabus: async (id, fileName) => {
    const { blob, mime, url } = await fetchBlob(`/api/vocational/silabus/${id}/view`);
  ...
  downloadSilabus: async (id, fileName) => {
    ...
    const a = document.createElement('a');
    a.href = url; a.download = fileName || 'silabus';
    a.click(); setTimeout(() => URL.revokeObjectURL(url), 5000);
  },
  ...
  viewLaporanKegiatan: async (id, fileName) => {
    const { blob, mime, url } = await fetchBlob(`/api/vocational/laporan-kegiatan/${id}/view`);
    const inlineTypes = ['image/jpeg','image/jpg','image/png','image/gif','image/webp','application/pdf'];
    if (inlineTypes.includes(mime)) {
      return { url, mime, inline: true };
    }
  ...
  downloadLaporanKegiatan: async (id, fileName) => {
    const { url } = await fetchBlob(`/api/vocational/laporan-kegiatan/${id}/download`);
    const a = document.createElement('a');
    a.href = url; a.download = fileName || 'laporan';
    a.click(); setTimeout(() => URL.revokeObjectURL(url), 5000);
  },
