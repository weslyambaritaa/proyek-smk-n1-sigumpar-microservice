...
function hitungNilaiAkhir(row) {
  if (row.nilai_akhir !== undefined && row.nilai_akhir !== null) {
    return Number(row.nilai_akhir).toFixed(2);
  }

  const t = Number(row.tugas) || 0;
  const k = Number(row.kuis) || 0;
  const u = Number(row.uts) || 0;
  const a = Number(row.uas) || 0;
  const p = Number(row.praktik) || 0;
...
export default function RekapNilaiPage() {
  ...
  const sorted = [...filtered].sort((a, b) => {
    let va =
      sortField === "nilai_akhir"
        ? Number(hitungNilaiAkhir(a))
        : (a[sortField] ?? "");
    let vb =
      sortField === "nilai_akhir"
        ? Number(hitungNilaiAkhir(b))
        : (b[sortField] ?? "");
  ...
  const rataRata = nilaiAkhirList.length
    ? (
        nilaiAkhirList.reduce((a, b) => a + b, 0) / nilaiAkhirList.length
      ).toFixed(2)
    : "-";
  const tertinggi = nilaiAkhirList.length
    ? Math.max(...nilaiAkhirList).toFixed(2)
    : "-";
  const terendah = nilaiAkhirList.length
    ? Math.min(...nilaiAkhirList).toFixed(2)
    : "-";
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 md:px-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-sm ml-4">
          Lihat dan analisis rekap nilai berdasarkan mapel, kelas, dan tahun
          ajar
        </p>
      </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 py-4 border-b border-gray-100 gap-3">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {namaMapel} · {selectedTahun} · {filtered.length} siswa
                </p>
              </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    ...
                    <th
                      className="px-4 py-3 text-left cursor-pointer hover:text-gray-800 transition-colors"
                    ...
                    {[
                      ...
                      <th
                        key={f}
                        className="px-4 py-3 text-center cursor-pointer hover:text-gray-800 transition-colors w-20"
                        onClick={() => handleSort(f)}
                      >
                        {f.replace("nilai_", "").toUpperCase()}{" "}
                        <SortIcon field={f} />
                      </th>
                    ))}
                    <th
                      className="px-4 py-3 text-center cursor-pointer hover:text-gray-800 transition-colors w-28"
                      ...
                      >
                        ...
                        {[
                          ...
                          <td key={f} className="px-4 py-3 text-center">
                            <span
                              className={`font-medium ${Number(row[f]) >= 70 ? "text-gray-700" : "text-red-500"}`}
                            >
                              {row[f] ?? "-"}
                            </span>
                          </td>
                        ...
                        <td className="px-4 py-3 text-center">
                          <span
                            className={`font-bold text-base ${Number(akhir) >= 75 ? "text-blue-600" : Number(akhir) >= 60 ? "text-yellow-600" : "text-red-500"}`}
                          >
                            {akhir}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-center">
                          <span
                            className={`text-xs font-bold px-2 py-1 rounded-full border ${predikat.color}`}
                          >
                            {predikat.label}
                          </span>
                        </td>
      ...
      {sudahCari && !loading && rows.length === 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-20 text-center text-gray-400">
          ...
          <p className="text-sm mt-1">
            Silakan input nilai terlebih dahulu di halaman Input Nilai
          </p>
        </div>
      )}
      ...
      {!sudahCari && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-20 text-center text-gray-400">
          ...
          <p className="font-medium text-gray-600">
            Pilih Mapel, Kelas, dan Tahun Ajar
          </p>
          <p className="text-sm mt-1">
            lalu klik Tampilkan Rekap untuk melihat nilai siswa
          </p>
        </div>
      )}
    </div>
