...
const KOMPONEN = [
  { key: "tugas",   label: "Tugas",   field: "nilai_tugas" },
  { key: "kuis",    label: "Kuis",    field: "nilai_kuis" },
  { key: "uts",     label: "UTS",     field: "nilai_uts" },
  { key: "uas",     label: "UAS",     field: "nilai_uas" },
...
function hitungNilaiAkhir(row, bobot) {
  ...
  const nilai =
    (Number(row.nilai_tugas)   || 0) * bobot.tugas +
    (Number(row.nilai_kuis)    || 0) * bobot.kuis +
    (Number(row.nilai_uts)     || 0) * bobot.uts +
    (Number(row.nilai_uas)     || 0) * bobot.uas +
    (Number(row.nilai_praktik) || 0) * bobot.praktik;
  ...
  return (
    <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 mb-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="font-bold text-amber-800 text-sm">⚖️ Pengaturan Bobot Nilai</h3>
            ...
            <div className="relative">
              <input type="number" min="0" max="100" value={bobot[k.key]}
                ...
                className={`w-full text-center px-2 py-2 rounded-xl border text-sm font-bold focus:outline-none focus:ring-2 transition-all pr-6 ${
                  isValid ? "border-gray-200 focus:ring-blue-400" : "border-red-300 focus:ring-red-400 bg-red-50"
                }`} />
...
export default function InputNilaiPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await academicApi.getSiswaByKelas({
        kelas_id: selectedKelas, mapel_id: selectedMapel || undefined, tahun_ajar: selectedTahun || undefined,
      });
      const data = Array.isArray(res?.data?.data) ? res.data.data : [];
      const filtered = searchNama ? data.filter(r => r.nama_lengkap?.toLowerCase().includes(searchNama.toLowerCase())) : data;
      setRows(filtered.map(r => ({ ...r, _dirty: false })));
      setPage(1);
    } catch { toast.error("Gagal memuat data siswa"); setRows([]); }
  ...
  const handleSimpan = async () => {
    ...
    if (!bobotValid)    { toast.error("Total bobot harus 100% sebelum menyimpan"); return; }
    setSaving(true);
    try {
      await academicApi.saveNilaiBulk({
        mapel_id: selectedMapel, kelas_id: selectedKelas, tahun_ajar: selectedTahun,
        nilai: rows.map(r => ({
          siswa_id: r.siswa_id,
          nilai_tugas: Number(r.nilai_tugas) || 0, nilai_kuis: Number(r.nilai_kuis) || 0,
          nilai_uts: Number(r.nilai_uts) || 0, nilai_uas: Number(r.nilai_uas) || 0,
  ...
  const handleLoadRiwayat = async () => {
    ...
    try {
      const res = await academicApi.getSiswaByKelas({ kelas_id: riwayatKelas, mapel_id: riwayatMapel || undefined, tahun_ajar: riwayatTahun });
  ...
  const handleLoadRekap = async () => {
    ...
    try {
      const res = await academicApi.getSiswaByKelas({ kelas_id: rekapKelas, mapel_id: rekapMapel || undefined, tahun_ajar: rekapTahun });
  ...
  const nilaiList = rekapData.map(r => Number(hitungNilaiAkhir(r, bobot)));
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                ...
                <div>
                  ...
                  <input type="text" value={searchNama} onChange={e => setSearchNama(e.target.value)}
                    onKeyDown={e => e.key === "Enter" && fetchData()} placeholder="Ketik nama..."
                    className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div className="flex gap-2">
            ...
            {sudahCari && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                  ...
                  {namaMapel && <span className="text-xs font-bold text-blue-600 bg-blue-50 px-3 py-1 rounded-full border border-blue-100">{namaMapel}</span>}
                </div>
                ...
                {loading ? (
                  ...
                  <div className="py-16 text-center text-gray-400"><div className="text-5xl mb-3">📋</div><p>Tidak ada siswa ditemukan</p></div>
                ) : (
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                          ...
                          {KOMPONEN.map(k => (
                            <th key={k.key} className="px-4 py-3 text-center w-24">
                              {k.label}
                              <div className="text-blue-500 font-normal normal-case text-xs">{bobot[k.key]}%</div>
                            </th>
                          ))}
                            ...
                            <tr key={row.siswa_id} className={`transition-colors ${row._dirty ? "bg-yellow-50/60" : "hover:bg-gray-50/70"}`}>
                              ...
                              {KOMPONEN.map(k => (
                                <td key={k.key} className="px-4 py-3 text-center">
                                  <input type="number" min="0" max="100" value={row[k.field]}
                                    onChange={e => handleNilaiChange(globalIdx, k.field, e.target.value)}
        ...
        {tab === "riwayat" && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            {loadingRiwayat ? (
              ...
              <div className="py-16 text-center text-gray-400"><p className="text-4xl mb-2">📋</p><p>Pilih filter dan klik Tampilkan</p></div>
            ) : (
              <>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {pagedRiwayat.map((r, i) => {
                        const akhir = hitungNilaiAkhir(r, bobot);
                        return (
                        ...
                        {rekapData.map((r, i) => {
                          ...
                          return (
                            <tr key={i} className="hover:bg-blue-50/30 transition-colors">
                              ...
                              {KOMPONEN.map(k => (
                                <td key={k.key} className={`px-5 py-3 text-center font-medium ${Number(r[k.field]) >= 70 ? "text-gray-700" : "text-red-500"}`}>
                                  {r[k.field] ?? "-"}
                                </td>
                              ))}
                              <td className="px-5 py-3 text-center">
