    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="px-6 py-4 border-b border-orange-100 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-gray-800">Tambah Monitoring</h2>
            <p className="text-xs text-gray-500">{pklData?.nama_siswa} · {pklData?.nama_perusahaan}</p>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="flex justify-between text-xs font-semibold text-gray-600 mb-2">
              ...
              <span className="text-orange-600 font-bold">{form.progres_saat_kunjungan}%</span>
            </label>
            <input
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Pantau progres dan rekam kunjungan siswa PKL</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <h2 className="font-semibold text-gray-700 text-sm uppercase tracking-wide">Siswa PKL Aktif</h2>
            {pklList.filter(p => p.status_approval === 'disetujui').length === 0 ? (
              ...
              pklList
                ...
                .map(pkl => (
                  <div key={pkl.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className="font-bold text-gray-800">{pkl.nama_siswa}</p>
                        <p className="text-xs text-gray-500">{pkl.kelas} · {pkl.nama_perusahaan}</p>
                      </div>
                      <button
                    ...
                    {(logByPKL[pkl.id] || []).length > 0 && (
                      <p className="text-xs text-gray-400 mt-2">
                        {(logByPKL[pkl.id] || []).length}x kunjungan · Terakhir: {formatDate((logByPKL[pkl.id] || [{}])[0].tanggal_kunjungan)}
                      </p>
                    )}
                  </div>
                ))
          ...
          <div className="space-y-4">
            <h2 className="font-semibold text-gray-700 text-sm uppercase tracking-wide">Riwayat Kunjungan Terbaru</h2>
            {monitoringLog.length === 0 ? (
              ...
              monitoringLog.slice(0, 10).map(log => (
                <div key={log.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 border-l-4 border-l-orange-400">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <p className="font-semibold text-gray-800 text-sm">{log.nama_siswa || `PKL #${log.pkl_id}`}</p>
                      <p className="text-xs text-gray-500 mb-2">{log.nama_perusahaan || ''} · {formatDate(log.tanggal_kunjungan)}</p>
                      <p className="text-sm text-gray-700">{log.catatan}</p>
                    </div>
                    <div className="ml-4 text-right">
                      <span className={`text-lg font-black ${log.progres_saat_kunjungan >= 80 ? 'text-emerald-600' : log.progres_saat_kunjungan >= 50 ? 'text-amber-500' : 'text-red-500'}`}>
                        {log.progres_saat_kunjungan}%
                      </span>
                    </div>
                  </div>
                  {log.nama_petugas && (
                    <p className="text-xs text-gray-400 mt-2">👤 {log.nama_petugas}</p>
                  )}
                </div>
              ))
            )}
      ...
      <MonitoringModal
        isOpen={isModalOpen}
        onClose={() => { setIsModalOpen(false); setSelectedPKL(null); }}
        onSaved={fetchData}
        pklData={selectedPKL}
      />
    </div>
