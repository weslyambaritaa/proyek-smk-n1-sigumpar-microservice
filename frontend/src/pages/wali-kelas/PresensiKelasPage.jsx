...
const STATUS_COLOR = {
  hadir: "bg-green-500 text-white", sakit: "bg-blue-500 text-white",
  izin: "bg-yellow-400 text-white", alpa: "bg-red-500 text-white", terlambat: "bg-orange-400 text-white"
};
...
export default function PresensiKelasPage() {
  ...
  const loadRiwayat = async () => {
    ...
    try {
      const res = await axiosInstance.get(`/api/academic/absensi-siswa?kelas_id=${selectedKelas}`);
  ...
  const tandaiSemua = (s) => {
    const m = {};
    siswaList.forEach((st) => { m[st.id_siswa ?? st.id] = { status: s, keterangan: "" }; });
    setAbsensi(m);
  };

  const stats = useMemo(() => {
    const s = { hadir: 0, sakit: 0, izin: 0, alpa: 0, terlambat: 0 };
    Object.values(absensi).forEach((v) => { if (v?.status) s[v.status] = (s[v.status] || 0) + 1; });
    return s;
  }, [absensi]);
  ...
  const handleSimpan = async () => {
    ...
    setSaving(true);
    try {
      const attendance = siswaList.map((st) => {
        const id = st.id_siswa ?? st.id;
        const a = absensi[id] || { status: "hadir", keterangan: "" };
        return { id_siswa: id, status: a.status || "hadir", keterangan: a.keterangan || "" };
      });
      await axiosInstance.post("/api/academic/attendance/bulk", {
        classId: selectedKelas, date: tanggal, subjectId: null, attendance
      });
  ...
  const riwayatGrouped = useMemo(() => {
    const map = {};
    riwayat.forEach((r) => {
      if (!map[r.tanggal]) map[r.tanggal] = { tanggal: r.tanggal, hadir: 0, sakit: 0, izin: 0, alpa: 0, terlambat: 0, total: 0 };
      map[r.tanggal][r.status] = (map[r.tanggal][r.status] || 0) + 1;
      map[r.tanggal].total++;
    });
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="bg-white border-b px-8 py-5">
        <h1 className="text-2xl font-bold text-gray-800 tracking-tight">PRESENSI KELAS</h1>
        <p className="text-sm text-gray-500 mt-0.5">Input kehadiran siswa untuk kelas Anda</p>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="font-bold text-gray-800">
                Daftar Siswa{namaKelas && <span className="text-gray-400 font-normal"> — {namaKelas}</span>}
              </h2>
            ...
            {loading ? (
              ...
              <div className="py-16 text-center text-gray-400"><p className="text-4xl mb-2">👥</p><p>Tidak ada siswa</p></div>
            ) : (
                    ...
                    return (
                      <tr key={id} className="hover:bg-gray-50/70 transition-colors">
                        ...
                        <td className="px-5 py-3">
                          {curr.status ? (
                            <span onClick={() => {
                              const idx = STATUS_OPTS.indexOf(curr.status);
                              setStatus(id, STATUS_OPTS[(idx + 1) % STATUS_OPTS.length]);
                            }} className={`inline-block px-3 py-1 rounded-full text-xs font-bold uppercase cursor-pointer ${STATUS_COLOR[curr.status]}`}>
                              {curr.status}
                            </span>
                          ) : (
                            <div className="flex flex-wrap gap-1">
                              {STATUS_OPTS.map((s) => (
                                <button key={s} onClick={() => setStatus(id, s)}
                                  className="text-[10px] font-bold uppercase px-2 py-0.5 rounded border border-gray-300 text-gray-500 hover:bg-gray-100">
                                  {s}
                                </button>
                              ))}
                            </div>
                        ...
                        <td className="px-5 py-3">
                          <input type="text" value={curr.keterangan || ""} onChange={(e) => setKet(id, e.target.value)}
                            placeholder="Catatan..."
                            className="border border-gray-200 rounded-lg px-3 py-1 text-sm w-40 focus:outline-none focus:ring-2 focus:ring-blue-300" />
                        </td>
                      </tr>
                    );
                  })}
            ...
            {!loading && siswaList.length > 0 && (
              <div className="px-6 py-4 border-t border-gray-100 flex justify-end bg-gray-50/50">
                <button onClick={handleSimpan} disabled={saving}
                  className="px-10 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold text-sm rounded-xl shadow-sm transition-all active:scale-95">
        ...
        {showRiwayat && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100">
              <h2 className="font-bold text-gray-800 text-sm uppercase tracking-wide">Riwayat Absensi Kelas</h2>
            </div>
            {loadingRiwayat ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {riwayatGrouped.map((r) => (
                    <tr key={r.tanggal} className="hover:bg-gray-50/70">
                      <td className="px-5 py-3 font-semibold text-gray-800">{r.tanggal}</td>
                      <td className="px-5 py-3 text-center text-green-600 font-bold">{r.hadir}</td>
                      <td className="px-5 py-3 text-center text-blue-600 font-bold">{r.sakit}</td>
                      <td className="px-5 py-3 text-center text-yellow-500 font-bold">{r.izin}</td>
                      <td className="px-5 py-3 text-center text-red-500 font-bold">{r.alpa}</td>
                      <td className="px-5 py-3 text-center text-orange-500 font-bold">{r.terlambat}</td>
                      <td className="px-5 py-3 text-center text-gray-600 font-semibold">{r.total}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
