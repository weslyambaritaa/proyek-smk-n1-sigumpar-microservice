...
export default function RefleksiPage() {
  ...
  const loadRiwayat = async () => {
    ...
    try {
      const res = await axiosInstance.get(`/api/academic/wali/refleksi?kelas_id=${selectedKelas}`);
      setRiwayat(Array.isArray(res.data?.data) ? res.data.data : []);
    } catch { setRiwayat([]); }
    finally { setLoadingRiwayat(false); }
  };

  useEffect(() => { loadRiwayat(); }, [selectedKelas]);
  ...
  const handleSimpan = async () => {
    ...
    if (!anyFilled) { toast.error("Isi minimal satu kolom refleksi"); return; }

    setSaving(true);
    try {
      await axiosInstance.post("/api/academic/wali/refleksi", {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="bg-white border-b px-8 py-5">
        <h1 className="text-2xl font-bold text-gray-800 tracking-tight">REFLEKSI</h1>
        <p className="text-sm text-gray-500 mt-0.5">Catatan refleksi pembelajaran mingguan wali kelas</p>
      </div>
      ...
      <div className="px-8 py-6 max-w-3xl mx-auto space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5">Tanggal Minggu Ini</label>
              <input type="date" value={minggu} onChange={(e) => setMinggu(e.target.value)}
          ...
          {REFLEKSI_TEMPLATE.map((t) => (
            <div key={t.id}>
              ...
              <textarea value={refleksi[t.id] || ""} onChange={(e) => setRefleksi((p) => ({ ...p, [t.id]: e.target.value }))}
        ...
        {selectedKelas && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
            <h3 className="font-bold text-gray-700 mb-3 text-sm uppercase tracking-wide">Riwayat Refleksi</h3>
            {loadingRiwayat ? (
              <div className="py-8 text-center text-gray-400">Memuat...</div>
            ) : riwayat.length === 0 ? (
              <div className="py-8 text-center text-gray-400 text-sm">Belum ada riwayat refleksi</div>
            ) : (
              <div className="space-y-3">
                {riwayat.map((r) => (
                  <div key={r.id} className="border border-gray-100 rounded-xl p-4 bg-gray-50">
                    <p className="text-xs font-semibold text-blue-600 mb-2">{r.tanggal}</p>
                    {REFLEKSI_TEMPLATE.map((t) => r[t.id] && (
                      <p key={t.id} className="text-sm text-gray-700 mb-1">
                        <span className="font-semibold">{t.label}:</span> {r[t.id]}
                      </p>
                    ))}
                  </div>
                ))}
