const { Kelas, MataPelajaran, Siswa } = require("../models");
...
exports.getAttendanceByClass = asyncHandler(async (req, res) => {
  ...
  const rows = await sequelize.query(
    ...
    {
      replacements: { classId, date, subjectId: subjectId || null },
...
exports.saveBulkAttendance = asyncHandler(async (req, res) => {
  ...
  if (!classId || !date || !Array.isArray(attendance))
    throw createError(400, "Data tidak lengkap");
  if (attendance.length === 0)
    return res.json({
      success: true,
      message: "Tidak ada absensi yang disimpan",
    });
