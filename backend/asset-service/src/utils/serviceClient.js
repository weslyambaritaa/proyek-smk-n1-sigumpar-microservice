...
class ServiceClient {
  constructor() {
    this.services = {
      academic: process.env.ACADEMIC_SERVICE_URL || 'http://academic-service:3003',
      auth: process.env.AUTH_SERVICE_URL || 'http://auth-service:3005',
      learning: process.env.LEARNING_SERVICE_URL || 'http://learning-service:3006',
      student: process.env.STUDENT_SERVICE_URL || 'http://student-service:3004',
      vocational: process.env.VOCATIONAL_SERVICE_URL || 'http://vocational-service:3007',
  ...
  // ── ACADEMIC SERVICE METHODS ──────────────────────────────────────────────

  async getSiswa(kelasId = null, authToken = null) {
    const params = kelasId ? { kelas_id: kelasId } : {};
    return this.callService('academic', '/api/academic/siswa', 'GET', {
      params,
      headers: authToken ? { Authorization: authToken } : {}
    });
  }

  async getKelas(authToken = null) {
    return this.callService('academic', '/api/academic/kelas', 'GET', {
      headers: authToken ? { Authorization: authToken } : {}
    });
  }

  async getGuru(searchQuery = '', authToken = null) {
    return this.callService('academic', '/api/academic/guru/search', 'GET', {
      params: { q: searchQuery },
      headers: authToken ? { Authorization: authToken } : {}
    });
  }

  async createKelas(kelasData, authToken) {
    return this.callService('academic', '/api/academic/kelas', 'POST', {
      data: kelasData,
  ...
  async getKepsekStatistik(authToken) {
    return this.callService('learning', '/api/learning/kepsek/statistik', 'GET', {
      headers: { Authorization: authToken }
    });
  }
  ...
  async getParentingData(waliId, authToken) {
    return this.callService('student', `/api/student/wali/parenting?wali_id=${waliId}`, 'GET', {
      headers: { Authorization: authToken }
    });
  }
  ...
  async getPKLData(siswaId, authToken) {
    return this.callService('vocational', `/api/vocational/pkl/${siswaId}`, 'GET', {
      headers: { Authorization: authToken }
    });
  }
}
