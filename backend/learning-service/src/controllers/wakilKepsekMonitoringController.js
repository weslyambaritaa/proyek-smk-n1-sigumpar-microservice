...
exports.getJadwalMonitoring = asyncHandler(async (req, res) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    for (let j = i + 1; j < rows.length; j++) {
      ...
      if (
        a.waktu_mulai < b.waktu_berakhir &&
        b.waktu_mulai < a.waktu_berakhir
      ) {
        bentrokIds.add(a.id);
        bentrokIds.add(b.id);
      }
...
exports.getParentingMonitoring = asyncHandler(async (req, res) => {
  ...
  const data = await ParentingLog.findAll({
    where,
    include: [
      { model: Kelas, foreignKey: "kelas_id", attributes: ["nama_kelas"] },
    ],
    order: [
      ["tanggal", "DESC"],
      ["id", "DESC"],
...
exports.getLaporanRingkas = asyncHandler(async (req, res) => {
  const [jadwalRes, kelasRes, guruRes, parentingRes, perangkatRes] =
    await Promise.all([
      sequelize.query(
        "SELECT COUNT(*) AS total, COUNT(DISTINCT guru_id) AS guru, COUNT(DISTINCT kelas_id) AS kelas FROM jadwal_mengajar",
        { type: QueryTypes.SELECT },
      ),
      sequelize.query("SELECT COUNT(*) AS total FROM kelas", {
        type: QueryTypes.SELECT,
      }),
      sequelize.query("SELECT COUNT(*) AS total FROM guru", {
        type: QueryTypes.SELECT,
      }),
      sequelize.query("SELECT COUNT(*) AS total FROM parenting_log", {
        type: QueryTypes.SELECT,
      }),
      sequelize
        .query(
  ...
  res.json({
    ...
    data: {
      jadwal: {
        total_jam: +jadwalRes[0].total,
        total_guru: +jadwalRes[0].guru,
        total_kelas: +jadwalRes[0].kelas,
      },
      kelas: { total: +kelasRes[0].total },
      guru: { total: +guruRes[0].total },
      perangkat: {
        total: +perangkatRes[0].total,
        lengkap: +perangkatRes[0].lengkap,
      },
