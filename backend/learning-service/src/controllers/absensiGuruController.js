...
const getAbsensiGuruById = async (req, res, next) => {
  try {
    const result = await pool.query(
      'SELECT * FROM absensi_guru WHERE id_absensi_guru = $1',
      [req.params.id]
    );
...
const createAbsensiGuru = async (req, res, next) => {
  try {
    ...
    if (!user_id || !namaGuru) {
      throw createError(400, 'Field user_id dan namaGuru wajib diisi');
    }
    ...
    const existing = await pool.query(
      'SELECT id_absensi_guru FROM absensi_guru WHERE user_id = $1 AND tanggal = $2',
      [user_id, tanggal]
    );
    ...
    const result = await pool.query(
      ...
      (user_id, nama_guru, mata_pelajaran, jam_masuk, tanggal, foto, status, keterangan)
...
const deleteAbsensiGuru = async (req, res, next) => {
  try {
    const result = await pool.query(
      'DELETE FROM absensi_guru WHERE id_absensi_guru = $1 RETURNING *',
      [req.params.id]
    );
