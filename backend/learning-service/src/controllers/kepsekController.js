...
exports.getEvaluasiGuru = async (_req, res) => {
...
exports.getKepsekDashboard = async (_req, res) => {
  const [absensi, perangkat, evaluasi] = await Promise.all([
    ...
    pool.query("SELECT COUNT(*)::int AS selesai, COALESCE(ROUND(AVG(COALESCE(skor,0))),0)::int AS rata FROM evaluasi_kinerja_guru WHERE LOWER(COALESCE(status,'')) LIKE '%selesai%'")
  ]);
