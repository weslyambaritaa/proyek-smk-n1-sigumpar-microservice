const { ParentingLog, Kelas, NilaiSiswa, Siswa, MataPelajaran, AbsensiSiswa } = require('../models');
...
exports.getParenting = asyncHandler(async (req, res) => {
  ...
  const data = await ParentingLog.findAll({
    where,
    include: [{ model: Kelas, foreignKey: 'kelas_id', as: 'kelas', attributes: ['nama_kelas'] }],
...
exports.createParenting = asyncHandler(async (req, res) => {
  ...
  const foto_url = req.file ? `/api/academic/uploads/${req.file.filename}` : null;
  const data = await ParentingLog.create({
    kelas_id: kelas_id || null,
    wali_id: wali_id || null,
    tanggal: tanggal || new Date().toISOString().slice(0, 10),
    kehadiran_ortu: kehadiran_ortu || 0,
    agenda, ringkasan: ringkasan || '', foto_url,
  });
...
exports.getKebersihan = asyncHandler(async (req, res) => {
  ...
  const { KebersijanKelas } = require('../models'); // lazy require karena model belum ditambahkan
...
exports.getRefleksi = asyncHandler(async (req, res) => {
  const conditions = [];
  const replacements = {};
  if (req.query.kelas_id) { conditions.push('kelas_id = :kelas_id'); replacements.kelas_id = req.query.kelas_id; }
  if (req.query.wali_id)  { conditions.push('wali_id = :wali_id');   replacements.wali_id  = req.query.wali_id; }
...
exports.createRefleksi = asyncHandler(async (req, res) => {
  ...
  if (!capaian && !tantangan && !rencana) throw createError(400, 'Minimal satu dari capaian, tantangan, atau rencana harus diisi');
  ...
  const [rows] = await sequelize.query(
    ...
    {
      replacements: {
        kelas_id: kelas_id || null, wali_id: wali_id || null,
        tanggal: tanggal || new Date().toISOString().slice(0, 10),
        capaian: capaian || '', tantangan: tantangan || '', rencana: rencana || '',
      },
      type: QueryTypes.SELECT,
    }
  );
...
exports.getRekapNilaiWali = asyncHandler(async (req, res) => {
  ...
  const rows = await sequelize.query(
    ...
    { replacements: { kelas_id, mapel_id: mapel_id || null, tahun_ajar: tahun_ajar || null }, type: QueryTypes.SELECT }
  );
  res.json({ success: true, data: rows });
});
...
exports.getRekapAbsensiWali = asyncHandler(async (req, res) => {
  ...
  const rows = await sequelize.query(
    ...
    { replacements: { kelas_id, bulan: bulan ? +bulan : null, tahun: tahun ? +tahun : null }, type: QueryTypes.SELECT }
  );
  res.json({ success: true, data: rows });
});