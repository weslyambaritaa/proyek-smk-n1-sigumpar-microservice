...
const getSiswaFromAcademic = async (kelas_id, authToken) => {
  try {
    const data = await serviceClient.getSiswa(kelas_id, authToken);
...
const getKelasFromAcademic = async (authToken) => {
  try {
    const data = await serviceClient.getKelas(authToken);
    return Array.isArray(data) ? data : data?.data || [];
  } catch (err) {
...
exports.createProgresPKL = asyncHandler(async (req, res) => {
  ...
  if (!siswa_id || !minggu_ke)
    throw createError(400, "siswa_id dan minggu_ke wajib diisi");

  const data = await LaporanProgresPKL.create({
    siswa_id,
    minggu_ke,
...
exports.getNilaiPKL = asyncHandler(async (req, res) => {
  ...
  const nilaiRows = await NilaiPKL.findAll({
    where,
    order: [["siswa_id", "ASC"]],
  });
  ...
  let siswaList = [];
  if (kelas_id) {
    siswaList = await getSiswaFromAcademic(
      kelas_id,
...
exports.getSiswaForVokasi = asyncHandler(async (req, res) => {
  const siswa = await getSiswaFromAcademic(
    req.query.kelas_id,
    req.headers["authorization"],
  );
