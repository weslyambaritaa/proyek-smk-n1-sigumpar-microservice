...
exports.upsertPenilaian = async (req, res) => {
  ...
  exports.upsertPenilaian = async (req, res) => {
    ...
    // Logika Perhitungan Nilai Akhir (Rata-rata komponen)
    const komponen = [disiplin, teknis, komunikasi, laporan, presentasi].map(
      Number,
    );
    const nilai_akhir = (
    ...
    // Penentuan Grade secara otomatis
    let grade = "E";
      ...
      const result = await pool.query(
        `
            ...
            DO UPDATE SET 
                ...
                status_penilaian = EXCLUDED.status_penilaian, updated_at = CURRENT_TIMESTAMP
            RETURNING *`,
        [
          submission_id,
          disiplin,
