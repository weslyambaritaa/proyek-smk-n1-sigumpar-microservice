...
exports.getProyekById = async (req, res) => {
  ...
  try {
    ...
    if (proyek.rowCount === 0) {
      return res
        .status(404)
        .json({ success: false, message: "Proyek tidak ditemukan" });
    }
...
exports.createProyek = async (req, res) => {
  ...
  if (!judul_proyek) {
    return res
      .status(400)
      .json({ success: false, message: "Judul proyek wajib diisi" });
  }
...
exports.deleteProyek = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM proyek_vokasi WHERE id = $1 RETURNING id",
      [id],
    );
    if (result.rowCount === 0) {
      return res
...
exports.addAnggotaProyek = async (req, res) => {
  ...
  if (!nama_siswa) {
    return res
      .status(400)
      .json({ success: false, message: "Nama siswa wajib diisi" });
  }
